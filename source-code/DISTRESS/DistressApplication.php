...
class DistressApplication extends distressApplicationStatic
{
    ...
    public function getEfficiencyLevel()
    {
        ...
        if ($networkStats->session->received) {
            // Let's assume that, if received traffic is 10 times large then transmitted traffic,
            // then we have 100% response rate

            $responseRate = roundLarge($networkStats->session->received / 10 / $networkStats->session->transmitted * 100);
            if ($responseRate > 100) {
                $responseRate = 100;
            }
            return $responseRate;
